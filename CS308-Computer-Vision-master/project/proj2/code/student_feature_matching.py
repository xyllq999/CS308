...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    # print(np.tile(features1[0, :], (m, 1)).shape)

    match_points = []
    for i in range(n):
        distance = np.sum(np.square(np.tile(features1[i, :], (m, 1)) - features2), 1)
        argnn = np.argpartition(distance, 2)[:2]
        # print(argnn)
        # print(distance[argnn[0]])
        # print(distance[argnn[1]])
